...
| V11 | Brightness-aware context tokens (chunk 1) | **Done** | agent | V08 follow-on. New `ThemeTokens` extension on `BuildContext` — `context.appColors` / `context.appTypography` resolve to brightness-correct token classes (`_DarkTokens` / `_LightTokens`) via `Theme.of(context).brightness`. Field-for-field parity with `AppColors`/`LightAppColors` and `AppTypography`/`LightAppTypography`. Migrated `login_screen.dart` + `tv_text_field.dart` (used by login + playlist). Dark theme still renders identically; light theme now actually changes the visible surfaces for the login + playlist connection forms. 9 new tests (6 theme_extensions: context.appColors picks correctly in dark/light, dark vs light constants distinct, typography picks correctly, body text distinct; 3 login_screen: renders dark, renders light, shows auth error), 141 total. 0 new analyze errors. **Merged to develop (23243dd) — CI ✅ (6m45s) + Release ✅ (6m40s).** |

---

> Last updated: 2026-06-09T17:30:00+01:00
